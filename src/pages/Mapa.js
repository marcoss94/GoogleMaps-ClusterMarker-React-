  ...
  const filtrar = () => {
    ...
    data_filter_Servicios.map((item) => {
      item.sectorID.forEach((element) => {
        ...
        if (!array.some((item) => item.id === aux.id)) array.push(aux);
      });
    ...
    if (arrayTiposSistemas.length !== 0) {
      ...
      data_filter_Sectores = data_filter_Sectores.filter((item) => {
        ...
        arrayTiposSistemas.forEach((element) => {
          if (
            String(item.id).startsWith(element.value) ||
            (!isNaN(parseInt(item.id)) && element.value === "#")
          ) {
            bool = true;
          }
        });
              ...
              if (p) {
                ...
                if (p.sectorID) {
                  p.sectorID.forEach((item) => {
                    let s = data_filter_Sectores.find(
                      (element) => element.id === item
                    );
                    if (!arraySector.some((aux) => aux.id === s.id))
                      arraySector.push(s);
                    s.servicioID.forEach((aux) => {
                      let s = data_filter_Servicios.find(
                        (element) => element.id === aux
                      );
    ...
    if (
      (arrayServicios.length === 0 &&
        arraySistemas.length !== 0 &&
        arrayMedidas.length === 0) ||
      (arrayServicios.length === 0 &&
        arrayTiposSistemas.length !== 0 &&
        arrayMedidas.length === 0)
    ) {
      // filtrado por servicio si se selecciono un filtro de servicio
      data_filter_Servicios = data_filter_Servicios.filter((item) => {
    ...
    <>
      ...
      {markers.length !== 0 && current_point && (
        <ShowModal
          ...
          sectores={data_filter_Sectores}
          puntos={data_filter_PuntosM}
        />
      )}
